...
class TestEEOUT:
    ...
    @pytest.mark.parametrize(['file', 'exp'],
                             [['cyl_tetra1.eeout', {'NUMBER OF PARTICLES': 1,
                                                    'NUMBER OF NODES': 391,
                                                    ...
                                                    'NUMBER OF EDITS': 2}],
                              ['cyl_tetra2.eeout', {'NUMBER OF PARTICLES': 1,
    ...
    def test_get_materials_name(self, file):
        with as_file(eeout_resources.joinpath(file)) as inp:
            eeout = EEOUT(inp)
        assert eeout._get_materials_name() == {1: 'water_001',
                                               2: 'steel_002'}

    @pytest.mark.parametrize(['file', 'exp_idx'],
                             [['cyl_tetra1.eeout', 334],
                              ['cyl_tetra2.eeout', 1552]])
    ...
    @pytest.mark.parametrize(['file', 'exp_idx'],
                             [['cyl_tetra1.eeout', 535],
                              ['cyl_tetra2.eeout', 1756]])
    ...
    @pytest.mark.parametrize(['file', 'exp_idx'],
                             [['cyl_tetra1.eeout', 1843],
                              ['cyl_tetra2.eeout', 4412]])
