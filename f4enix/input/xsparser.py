...
def get_xs(mat_nuc, mt: int, lib: str, lib_manager: LibManager, material: bool = True):

    if material:
        energies, xs, fractions = get_material_xs(
            mat_nuc, mt, lib, lib_manager=lib_manager
        )
...
def get_zaid_xstable(zaid_name: str, lib: str, lib_manager: LibManager):
    ...
    for xs_tab in lib_manager.XS.tables:
        if zaid_name == xs_tab.name:
            break
    filename = xs_tab.filename.replace("/", "\\")
    if filename.startswith("\\"):
        filename = filename.lstrip("\\")
    filepath = os.path.join(lib_manager.XS.directory, filename)
...
def _build_material_xs(energies: list, xs: list, fractions: list):
    ...
    for l, xs_ in enumerate(xs):
        if l == 0:
            ...
            mat_xs = mat_xs + fractions[l] * np.interp(
                combined_energies, energies[l], xs_
            )

    return combined_energies, mat_xs / sum(fractions)


def partial_energy_matrix_mono(E_g: np.ndarray, E_n: np.ndarray, slope: int = -1):
...
def pointwise_collapse(x_g, x, y, logx=False, logy=False, log=False):
    ...
    if not (np.all(np.diff(x) >= 0.0) or np.all(np.diff(x) <= 0)):
        raise ValueError("x must be monotonically " "increasing/decreasing.")
    if not (np.all(np.diff(x_g) >= 0.0) or np.all(np.diff(x_g) <= 0)):
