...
class Tally:
    ...
    def __init__(self, tN: int) -> None:
        ...
        self.detectorTypeList = {
                ...
                # is used
                -3: "Spherical mesh tally", -2: "Cylindrical mesh tally",
        ...
        self.particleListShort = {
                1: "Neutron", 2: "Photon", 3: "Neutron + Photon",
                4: "Electron", 5: "Neutron + Electron",
                6: "Photon + Electron", 7: "Neutron + Photon + Electron"}

        self.particleList = (
                #   1         2          3          4      5
                "Neutron", "Photon", "Electron", "Muon", "Tau",
                #         6                 7
                "Electron Neutrino", "Muon Neutrino",
                #     8            9          10         11         12
                "Tau Neutrino", "Proton", "Lambda 0", "Sigma +", "Sigma -",
                #   13           14          15          16
                "Cascade 0", "Cascade -", "Omega -", "Lambda c +",
                #   17              18              19         20
                "Cascade c +", "Cascade c 0", "Lambda b 0", "Pion +",
                #   21             22           23        24
                "Neutral Pion", "Kaon +", "K0 Short", "K0 Long",
                #  25   26     27       28     29      30        31
                "D +", "D 0", "D s +", "B +", "B 0", "B s 0", "Deuteron",
                #  32      33       34              35
                "Triton", "He3", "He4 (Alpha)", "Heavy ions")

        self.binIndexList = ("f", "d", "u", "s", "m", "c", "e", "t", "i", "j",
                             "k")
    ...
    def _initializeValuesVectors(self) -> None:
        ...
        self.valsErrors = np.empty(
                (nCells, nDir, nUsr, nSeg, nMul, nCos, nErg, nTim,
                 nCora, nCorb, nCorc, 2), dtype=float)
    ...
    def _parseTally(self) -> bool:
                ...
                while self.line[0].lower() != "d": # CELLS
                        if tally.mesh:
                                ...
                                for c in self.line.split():
                                        if not tally._insertCell(int(c)):  # This means that for some reason you are trying to
                                                                                        ...
                                                                                        for j in range(nCorb):
                                                                                                for i in range(nCora):
                                                                                                        ...
                                                                                                        if self.line[0:3] != "tfc":
                                                                                                                ...
                                                                                                                tally._insertValue(c,d,u,s,m,a,e,t,i,j,k,0,val)
                                                                                                                tally._insertValue(c,d,u,s,m,a,e,t,i,j,k,1,err)
                                            ...
                                            for k, kn in enumerate(binnings['cor C']):
                                                for j, jn in enumerate(binnings['cor B']):
                                                    for i, ina in enumerate(binnings['cor A']):
                                                        val = t._getValue(f, d, u, sn-1, m, c, e, nt, i, j, k, 0)
                                                        err = t._getValue(f, d, u, sn-1, m, c, e, nt, i, j, k, 1)
